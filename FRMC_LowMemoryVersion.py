...
This LowMemoryVersion is an upgrade to the previous version, which significantly 
...
def square_to_condensed(i, j, n):
    #n is square n-by-n symmetric distance matrix
...
def obtainSquarei(v, i):
    #get the i row of distance matrix from vector v
    #return value res == dist.squareform( v )[i]
    s=len(v)
    d = int(np.ceil(np.sqrt(s * 2))) #d-by-d distance matrix
    if d * (d - 1) != s * 2:
    ...
    for j in range(0,d):
        if i !=j:
            res[j]=v[square_to_condensed(i,j,d)]
    res[i]=1 # diagonal is 1
    return res
...
def getDropout(M, t=0.2):
    ...
    v= 1.0 - dist.pdist(M0, 'jaccard')# this is Jac index
    Jpv = v.copy() #keep jaccard value
    v[v >= 0.5] = 1 # this cells are similar
    v[v < 0.5] = 0
    #J[i]==obtainSquarei(v,i)
    ...
    for i in range(m):
        ...
        #p_down = np.sum(J[i])
        Ji=obtainSquarei(v,i)
        p_down = np.sum(Ji)
        if p_down < 10:
            #降序
            Jpi=obtainSquarei(Jpv,i)
            Ji[ np.argsort(-Jpi)[0:10] ]=1
